...
@vertex fn vertex(@location(0) position:vec3<f32>,@location(1) uv:vec2<f32>,@location(2) layer:f32,@location(3) color:vec3<f32>)->VertexOut {
 ...
 let near=30.0;let far=2200000.0;let f=1.7320508;
 var out:VertexOut;
...
@fragment fn fragment(in:VertexOut)->@location(0) vec4<f32>{
 ...
 if in.layer>=0.0 {
  let tex=textureSample(tiles,tile_sampler,in.uv,i32(in.layer));
...
@fragment fn sky_fragment(in:SkyOut)->@location(0) vec4<f32>{
 let ray=normalize(scene.forward.xyz+scene.right.xyz*in.screen.x*scene.eye.w/1.7320508+scene.up.xyz*in.screen.y/1.7320508);
