...
fn actions() -> Vec<String> {
    let mut a="pitch roll yaw throttle throttle-rate look-x look-y gear flaps airbrake hook bay engine burner radar jammer autopilot waypoint-autopilot fire weapon-seeker-mode weapon-next weapon-previous designate clear-designation jettison range-target damage-class fail-station damage-player target-jammer incoming pause menu end-flight restart view-front view-back view-up view-external center-look cockpit hud zoom-in zoom-out range-down range-up radar-mode sensor-channel sensor-infrared sensor-history airport-nav airport-next airport-request-landing airport-repeat airport-cancel instrument-next instrument-previous menu-up menu-down menu-left menu-right menu-accept menu-back".split_whitespace().map(str::to_owned).collect::<Vec<_>>();
    for n in 0..10 {
