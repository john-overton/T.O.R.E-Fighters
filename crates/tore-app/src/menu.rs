...
impl Menu {
    ...
    pub fn new(mut assets: Assets, selected: Option<&str>) -> crate::AppResult<Self> {
        ...
        let mut sprites: BTreeMap<String, Sprite> = assets
            .pics
            .into_iter()
            .map(|(name, p)| {
            ...
            .collect();
        // The version label is drawn in the bundled open-licensed font rather
...
/// Cell size of `menu-font-large.bin`: 24 by 17 pixels, Noto Sans Medium at
/// 14 px. Wide enough for every ASCII advance and tall enough for `{` and `j`.
pub(crate) const LARGE_CELL: (usize, usize) = (24, 17);
/// The larger bundled atlas. Text drawn with it survives the 1.5x to 3x
/// nearest-neighbour upscale to a 1080p or fullscreen window, which the 11 px
/// atlas does not. `flat_font` is unchanged for callers laid out around it.
pub(crate) fn flat_font_large(color: [u8; 3]) -> Sprite {
    // Open-licensed Noto Sans Medium, generated by tools/build_menu_font.py.
    let atlas = include_bytes!("../assets/menu-font-large.bin");
    let (cell_w, cell_h) = LARGE_CELL;
    Sprite {
        width: 256 * cell_w,
        height: cell_h,
        glyphs: (0..256)
            .map(|code| [code * cell_w, atlas[code] as usize, cell_h])
            .collect(),
        rgba: atlas[256..]
            .iter()
            .flat_map(|&a| [color[0], color[1], color[2], a])
    ...
    pub(crate) fn text(
        ...
        for c in text.bytes() {
            ...
            if let Some(rgb) = tint {
                ...
                // with what is under it exactly as `blit` does. Writing the
                // tint at full opacity instead loses every antialiased edge
                // and is what made scaled-up text look blocky.
                for yy in 0..h {
                    for xx in 0..w {
                        let at = (yy * font.width + sx + xx) * 4;
                        let alpha = font.rgba[at + 3] as u32;
                        if alpha == 0 {
                            continue;
                        }
                        let (dx, dy) = (x + xx as i32, y + yy as i32);
                        ...
                        let dest = (dy as usize * WIDTH + dx as usize) * 4;
                        for (c, tint) in rgb.iter().enumerate() {
                            let shade = (*tint as u32 * font.rgba[at + c] as u32 + 127) / 255;
                            self.0[dest + c] =
                                ((shade * alpha + self.0[dest + c] as u32 * (255 - alpha) + 127)
                                    / 255) as u8;
                        }
                        self.0[dest + 3] = 255;
                    }
