//! SHA-256 for source-build identification; not used for authentication.
pub(super) fn sha256(data: &[u8]) -> String {
    ...
    let length = (data.len() as u64 * 8).to_be_bytes();
    let total = (data.len() + 9).div_ceil(64) * 64;
    for offset in (0..total).step_by(64) {
        ...
        for (v, n) in h.iter_mut().zip([a, b, c, d, e, f, g, hh]) {
            *v = v.wrapping_add(n);
        }
    }
