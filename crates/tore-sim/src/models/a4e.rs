//! A-4E-owned fitted laws. Source configuration with an independently owned shared response fit.
...
pub struct A4EFlightModel {
    pub(super) configuration: Arc<Configuration>,
}
impl A4EFlightModel {
    pub fn from_aircraft(a: &tore_formats::aircraft::Aircraft) -> tore_formats::Result<Self> {
        let supported = a.id == tore_formats::aircraft::AircraftId::A4E
            && a.name == "A-4E"
            && a.shape == "A4.SH";
        if !supported {
            return Err(std::io::Error::other(
                "aircraft identity does not match A4EFlightModel",
            ));
        }
        let tuning = Tuning {
...
impl FlightModel for A4EFlightModel {
