//! MiG-23-owned fitted laws. Source configuration with an independently owned shared response fit.
...
pub struct Mig23FlightModel {
    pub(super) configuration: Arc<Configuration>,
}
impl Mig23FlightModel {
    pub fn from_aircraft(a: &tore_formats::aircraft::Aircraft) -> tore_formats::Result<Self> {
        let supported = a.id == tore_formats::aircraft::AircraftId::Mig23
            && a.name == "MiG-23"
            && a.shape == "MIG23.SH";
        if !supported {
            return Err(std::io::Error::other(
                "aircraft identity does not match Mig23FlightModel",
            ));
        }
        let tuning = Tuning {
...
impl FlightModel for Mig23FlightModel {
