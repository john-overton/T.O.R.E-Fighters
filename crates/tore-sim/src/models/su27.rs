//! Su-27-owned fitted laws. Source configuration with an independently owned shared response fit.
...
pub struct Su27FlightModel {
    pub(super) configuration: Arc<Configuration>,
}
impl Su27FlightModel {
    pub fn from_aircraft(a: &tore_formats::aircraft::Aircraft) -> tore_formats::Result<Self> {
        let supported = a.id == tore_formats::aircraft::AircraftId::Su27
            && a.name == "Su-27"
            && a.shape == "SU27.SH";
        if !supported {
            return Err(std::io::Error::other(
                "aircraft identity does not match Su27FlightModel",
            ));
        }
        let tuning = Tuning {
...
impl FlightModel for Su27FlightModel {
