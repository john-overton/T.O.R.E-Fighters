//! Su-25-owned fitted laws. Source configuration with an independently owned shared response fit.
...
pub struct Su25FlightModel {
    pub(super) configuration: Arc<Configuration>,
}
impl Su25FlightModel {
    pub fn from_aircraft(a: &tore_formats::aircraft::Aircraft) -> tore_formats::Result<Self> {
        let supported = a.id == tore_formats::aircraft::AircraftId::Su25
            && a.name == "Su-25"
            && a.shape == "SU25.SH";
        if !supported {
            return Err(std::io::Error::other(
                "aircraft identity does not match Su25FlightModel",
            ));
        }
        let tuning = Tuning {
        ...
        let equipment = Equipment {
            ...
            ground_clearance_ft: 7.666666666667,
        };
        Ok(Self {
...
impl FlightModel for Su25FlightModel {
