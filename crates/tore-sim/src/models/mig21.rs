//! MiG-21-owned fitted laws. Source configuration with an independently owned shared response fit.
...
pub struct Mig21FlightModel {
    pub(super) configuration: Arc<Configuration>,
}
impl Mig21FlightModel {
    pub fn from_aircraft(a: &tore_formats::aircraft::Aircraft) -> tore_formats::Result<Self> {
        let supported = a.id == tore_formats::aircraft::AircraftId::Mig21
            && a.name == "MiG-21"
            && a.shape == "MIG21.SH";
        if !supported {
            return Err(std::io::Error::other(
                "aircraft identity does not match Mig21FlightModel",
            ));
        }
        let tuning = Tuning {
        ...
        let equipment = Equipment {
            ...
            ground_clearance_ft: 6.000000000000,
        };
        Ok(Self {
...
impl FlightModel for Mig21FlightModel {
