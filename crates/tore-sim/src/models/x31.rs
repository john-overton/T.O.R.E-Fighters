//! X-31-owned fitted laws. Source configuration with an independently owned shared response fit.
//! Shared initial fit is an agent choice documented in docs/spec/additional-aircraft.md.
use super::{
...
pub struct X31FlightModel {
    pub(super) configuration: Arc<Configuration>,
}
impl X31FlightModel {
    pub fn from_aircraft(a: &tore_formats::aircraft::Aircraft) -> tore_formats::Result<Self> {
        let supported = a.id == tore_formats::aircraft::AircraftId::X31
            && a.name == "X-31"
            && a.shape == "F31.SH";
        if !supported {
            return Err(std::io::Error::other(
                "aircraft identity does not match X31FlightModel",
            ));
        }
        let tuning = Tuning {
...
impl FlightModel for X31FlightModel {
