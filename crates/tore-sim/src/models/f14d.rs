//! F-14-owned fitted laws. Source configuration with an independently owned shared response fit.
...
pub struct F14DFlightModel {
    pub(super) configuration: Arc<Configuration>,
}
impl F14DFlightModel {
    pub fn from_aircraft(a: &tore_formats::aircraft::Aircraft) -> tore_formats::Result<Self> {
        let supported = a.id == tore_formats::aircraft::AircraftId::F14
            && a.name == "F-14"
            && a.shape == "F14.SH";
        if !supported {
            return Err(std::io::Error::other(
                "aircraft identity does not match F14DFlightModel",
            ));
        }
        let tuning = Tuning {
...
impl FlightModel for F14DFlightModel {
