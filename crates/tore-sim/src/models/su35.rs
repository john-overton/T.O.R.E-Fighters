//! Su-35-owned fitted laws. Source configuration with an independently owned shared response fit.
...
pub struct Su35FlightModel {
    pub(super) configuration: Arc<Configuration>,
}
impl Su35FlightModel {
    pub fn from_aircraft(a: &tore_formats::aircraft::Aircraft) -> tore_formats::Result<Self> {
        let supported = a.id == tore_formats::aircraft::AircraftId::Su35
            && a.name == "Su-35"
            && a.shape == "SU35.SH";
        if !supported {
            return Err(std::io::Error::other(
                "aircraft identity does not match Su35FlightModel",
            ));
        }
        let tuning = Tuning {
        ...
        let equipment = Equipment {
            ...
            ground_clearance_ft: 7.333333333333,
        };
        Ok(Self {
...
impl FlightModel for Su35FlightModel {
