...
/// Seconds to full blackout at 9 G; higher G is proportionally faster.
...
/// Seconds to full redout at -3.5 G.
...
impl GEffects {
    ...
    pub fn step(&mut self, g: f64, enabled: bool) {
        ...
        self.blackout = if g > BLACKOUT_ONSET_G {
        ...
        self.redout = if g < REDOUT_ONSET_G {
...
mod tests {
    ...
    fn blackout_and_redout_follow_the_proposed_timings() {
        assert!((seconds_to_full(9.) - 5.).abs() < 0.02);
        assert!(seconds_to_full(10.5) < 5.);
        assert!((seconds_to_full(-3.5) - 3.).abs() < 0.02);
        let mut e = GEffects::default();
        for _ in 0..1200 {
            e.step(5.9, true);
            e.step(-1.9, true);
