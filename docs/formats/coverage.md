...
| ESA | Decoded | Bounded `SETUP.ESA` directory reader with synthetic tests; on the supplied disc 1 all 23 entries parsed and decoded to their declared sizes, `FA.EXE` matched its recorded SHA-256, and the four stored `FA_LIBS` archives were read in place with the same resource counts as the extracted files (1,996 / 5,405 / 77 / 22). Other pressings and DCL headers other than the reviewed ones are reported as unsupported |
...
FA F14.PT (636), A4E.PT (612) and F31.PT (660) share the reviewed 219-field
