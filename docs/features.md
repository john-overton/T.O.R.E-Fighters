...
| Expanded flight HUD and startup modes | ☐ | ☑ | Implemented across the selectable roster: expanded ladder window trimmed by 20%, lower weapon rows, wider bank arc, 25% tighter ladder spacing, lower boxed TAS/MSL values without surrounding numbers or hash marks, no fixed aircraft datum, and AGL/VS only for active ILS. Ground starts use NAV; airborne starts select the canonical gun with master arm SAFE. | [Layout and defaults](spec/hud-layout.md) |
