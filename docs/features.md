...
| AI formation and wing orders | ☑ | ☐ | Partially implemented. Each wing has its own leader and idle delta-formation following, with fitted signed speed correction, bank-driven formation turns, and render timing shared with the player. Formation geometry and the isolated receiver contract are implemented. Automatic requests and player break, engage, disengage and formation commands are routed within the intended wing. Broader mission orders remain. The approach steering point and several radio phrases remain unrecovered. | [B43, B46](spec/ai.md#b43-wing-commands-and-formation-variation) |
