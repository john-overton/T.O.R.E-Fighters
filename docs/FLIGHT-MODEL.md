...
with `--headless-flight 7200 --maneuver loop`. The rendered aircraft remains the
F/A-18D; **Rafale C has extraction and headless flight-model support, not a reviewed
cockpit/animation/rendering port**. No F18.SH animation addresses are applied to RAF.SH.
